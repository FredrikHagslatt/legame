...
void Garden::LoadScene()
{
    ...
    m_registry->emplace<Transform>(player, vec2f(200.0, 400.0), vec2f(3.0, 3.0), 0.0);
    ...
    m_registry->emplace<KeyboardControlled>(player, vec2f(0, -200), vec2f(200, 0), vec2f(0, 200), vec2f(-200, 0));
    m_registry->emplace<ProjectileEmitter>(player, vec2f(300.0, 300.0), 0, 10000, 10, true);
